...
{q:"Qual é a complexidade de busca em uma árvore binária de busca balanceada?",o:["O(n²)","O(log n)","O(1)","O(n)"],a:1},
{q:"O que o padrão Iterator implementa em Python (__iter__ e __next__)?",o:["Loop fixo","Protocolo para percorrer coleção sem expor estrutura interna","Filtering","Sorting"],a:1},
...
{q:"Quais são as 4 condições necessárias para deadlock (Coffman)?",o:["Exclusão mútua, posse e espera, sem preempção, espera circular","Mutex, semáforo, monitor, barrier","FCFS, SJF, RR, Priority","Race condition, starvation, thrashing, paging"],a:0},
{q:"O que a Lei de Amdahl limita?",o:["Speedup máximo: limitado pela fração serial do programa","Memória máxima","Número de threads","Tamanho do programa"],a:0},
{q:"O que o princípio Open/Closed (SOLID) diz?",o:["Aberto para extensão, fechado para modificação","Classes nunca mudam","Classes devem ser abertas","Aberto e fechado ao mesmo tempo"],a:0},
{q:"O que o Halting Problem prova?",o:["Existe problema que nenhum algoritmo pode resolver (indecidível)","Programas sempre terminam","P = NP","Compiladores são perfeitos"],a:0},
{q:"O que o gradiente descendente minimiza?",o:["A função de custo/loss atualizando parâmetros na direção oposta ao gradiente","Dados","Variância","Número de features"],a:0},
{q:"O que a maldição da dimensionalidade causa?",o:["Menos overfitting","Mais velocidade","Dados ficam esparsos em alta dimensão, degradando performance de modelos","Mais precisão"],a:2},
{q:"O que N+1 query problem é?",o:["Para cada item de uma lista, fazer query separada ao banco (deveria ser JOIN/batch)","Query lenta","Erro de SQL","Problema de cache"],a:0},
{q:"O que vertex shader e fragment shader fazem respectivamente?",o:["Mesmo trabalho","Vertex faz cor; fragment faz posição","Vertex é CPU; fragment é GPU","Vertex processa posição de vértices; fragment processa cor de cada pixel"],a:3},
{q:"O que é state management e por que é crucial em apps mobile?",o:["Gerenciar banco","Controlar dados compartilhados entre telas evitando inconsistências e re-renders","Gerenciar rotas","Gerenciar CSS"],a:1},
{q:"O que é canary deployment?",o:["Liberar nova versão para pequena % de usuários antes do rollout completo","Deploy azul","Deploy para pássaros","Rollback automático"],a:0},
{q:"O que SQL injection explora?",o:["Buffer overflow","Input não sanitizado que permite executar SQL malicioso no banco","Bugs de CSS","XSS"],a:1},
{q:"O que DMA (Direct Memory Access) faz?",o:["Transfere dados entre periférico e memória sem usar CPU (eficiente para I/O)","Acessa disco","Debugga código","Aloca memória"],a:0},
{q:"Em um sistema Full-Stack com Docker, qual a ordem correta de inicialização?",o:["Tudo junto","Frontend → Backend → Banco","Banco → Backend → Frontend (dependências de baixo para cima)","Não importa a ordem"],a:2},
{q:"O que microsserviços resolvem que monolitos não resolvem bem?",o:["Escalar e deployar partes independentemente, equipes autônomas","São mais simples","Usam menos recursos","São sempre melhores"],a:0},
{q:"Por que testes são importantes em CI/CD?",o:["Garantem que mudanças não quebram funcionalidade existente antes do deploy automático","Deixam o código bonito","Só para empresas grandes","São opcionais"],a:0},
{q:"O que containerização resolve comparado a 'funciona na minha máquina'?",o:["Nada","Só velocidade","Ambiente idêntico em dev, staging e produção (dependências empacotadas)","Só segurança"],a:2},
{q:"Qual é o papel de um message broker (RabbitMQ, Kafka) em sistemas distribuídos?",o:["Balanceia carga","Roteia HTTP","Desacopla serviços: comunicação assíncrona via filas/tópicos de mensagens","Armazena dados"],a:2}
];
